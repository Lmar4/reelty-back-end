...
const REQUIRED_TEMPLATES: TemplateKey[] = ALL_TEMPLATES;
...
export class ProductionPipeline {
      ...
      await videoProcessingService.stitchVideos(
        ...
        outputPath,
        {
          ...
          watermark: options.watermark
            ? {
                path: "assets/watermark/reelty_watermark.png",
                position: {
  ...
  protected async processTemplates(
    ...
    if (!templateMapVideo && coordinates) {
      const needsMap = REQUIRED_TEMPLATES.some((key) =>
    ...
    const templateResults = await Promise.all(
      REQUIRED_TEMPLATES.map(async (template) => {
        try {
          const result = await this.processTemplate(template, runwayVideos, {
            jobId,
            coordinates,
          ...
          logger.error(
            `[${jobId}] Template ${template} processing failed:`,
            error
          );
          return null;
        }
    ...
    try {
      ...
      if (cached) {
        ...
        if (cached.path.startsWith("http") || cached.path.startsWith("s3://")) {
          logger.info(
            `[${jobId}] Using cached map video from S3: ${cached.path}`
          );
          return cached.path;
        }
        ...
        try {
          await fs.access(cached.path);
          logger.info(
            `[${jobId}] Using cached map video from disk: ${cached.path}`
          );
          return cached.path;
        } catch (error) {
          logger.warn(
            `[${jobId}] Cached map video not found, will regenerate`,
            {
              path: cached.path,
              error: error instanceof Error ? error.message : "Unknown error",
            }
      ...
      const mapVideo = await this.withRetry(async () => {
        const video = await mapCaptureService.generateMapVideo(
          coordinates,
          jobId
        );
      ...
      if (process.env.UPLOAD_MAPS_TO_S3 === "true") {
        try {
          ...
          logger.warn(
            `[${jobId}] Failed to upload map video to S3, using local path`,
            {
              error: error instanceof Error ? error.message : "Unknown error",
            }
          );
