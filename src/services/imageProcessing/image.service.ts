...
export class ImageProcessor {
  ...
  public async convertToWebP(buffer: Buffer, options: ProcessingOptions = {}): Promise<Buffer> {
    return this.retryOperation(async () => {
      const image = sharp(buffer, {
        failOnError: true,
        limitInputPixels: 7680 * 7680, // Runway's max resolution
      });
      ...
      // Only resize if image exceeds Runway's limits
      if (metadata.width > 7680 || metadata.height > 7680) {
        ...
        if (metadata.width > metadata.height) {
          newWidth = 7680;
          newHeight = Math.round(7680 / aspectRatio);
        } else {
          newHeight = 7680;
          newWidth = Math.round(7680 * aspectRatio);
        }
      ...
      return image.webp({
        quality: 95,           // Good trade off between quality and size
        effort: 6,             // Maximum compression effort
        nearLossless: true,    // Enable near-lossless mode
        smartSubsample: false, // Disable subsampling to prevent graininess
        mixed: false,          // Disable mixed compression
        reoptimize: true      // Re-optimize the image
      }).toBuffer();
    });
  }
