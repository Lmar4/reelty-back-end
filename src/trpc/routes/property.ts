...
const initializePropertyRouter = async () => {
  const { router, protectedProcedure, publicProcedure } = await getRouteUtils();
  ...
  return router({
    getProperties: publicProcedure
      ...
      .query(async ({ input }) => {
        const { userId, status, limit = 10, cursor } = input;
...
export const getPropertyRouter = async () => {
  return await initializePropertyRouter();
};
